-- awr_io_ts.sql
-- AWR Tablespace IO Report, across SNAP_IDs
...
ttitle center 'AWR Tablespace IO Report' skip 2
set pagesize 50000
set linesize 250
...
select snap_id, TO_CHAR(tm,'YY/MM/DD HH24:MI') tm, inst, dur, tsname, io_rank, readtim, reads, atpr, rps, bpr, writetim, writes, atpw, wps, bpw, waits, atpwt, ios, iops
from 
      (select snap_id, tm, inst, dur, tsname, readtim, reads, atpr, rps, bpr, writetim, writes, atpw, wps, bpw, waits, atpwt, ios, iops,
            DENSE_RANK() OVER (
      ...
      from 
              ...
              select 
                                    ...
                                    e.tsname                                                tsname,
                                    sum(e.readtim  - nvl(b.readtim,0))                      readtim,
                                    sum(e.phyrds - nvl(b.phyrds,0))                         reads,
                                    decode(sum(e.phyrds - nvl(b.phyrds,0)), 0, 0, (sum(e.readtim - nvl(b.readtim,0)) / sum(e.phyrds  - nvl(b.phyrds,0)))*10)        atpr,
                                    sum(e.phyrds - nvl(b.phyrds,0)) / 
                                            ((round(EXTRACT(DAY FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) * 1440 
                                            + EXTRACT(HOUR FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) * 60 
                                            + EXTRACT(MINUTE FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) 
                                            + EXTRACT(SECOND FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) / 60, 2))*60) rps,      -- ela
                                    decode(sum(e.phyrds - nvl(b.phyrds,0)), 0, 0, sum(e.phyblkrd - nvl(b.phyblkrd,0)) / sum(e.phyrds   - nvl(b.phyrds,0)))              bpr,
                                    sum(e.writetim  - nvl(b.writetim,0))                 writetim,
                                    sum(e.phywrts    - nvl(b.phywrts,0))                   writes,
                                    decode(sum(e.phywrts - nvl(b.phywrts,0)), 0, 0, (sum(e.writetim - nvl(b.writetim,0)) / sum(e.phywrts  - nvl(b.phywrts,0)))*10)        atpw,
                                    sum(e.phywrts    - nvl(b.phywrts,0)) / 
                                            ((round(EXTRACT(DAY FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) * 1440 
                                            + EXTRACT(HOUR FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) * 60 
                                            + EXTRACT(MINUTE FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) 
                                            + EXTRACT(SECOND FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) / 60, 2))*60) wps,      -- ela
                                    decode(sum(e.phywrts - nvl(b.phywrts,0)), 0, 0, sum(e.phyblkwrt - nvl(b.phyblkwrt,0)) / sum(e.phywrts   - nvl(b.phywrts,0)))              bpw,
                                    sum(e.wait_count - nvl(b.wait_count,0))                waits,
                                    decode(sum(e.wait_count - nvl(b.wait_count, 0)), 0, 0, 10 * (sum(e.time - nvl(b.time,0)) / sum(e.wait_count - nvl(b.wait_count,0)))) atpwt,
                                    sum(e.phyrds  - nvl(b.phyrds,0)) + sum (e.phywrts - nvl(b.phywrts,0))                     ios,
                                    (sum(e.phyrds  - nvl(b.phyrds,0)) + sum (e.phywrts - nvl(b.phywrts,0))) / 
                                            ((round(EXTRACT(DAY FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) * 1440 
                                            + EXTRACT(HOUR FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) * 60 
                                            + EXTRACT(MINUTE FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) 
                ...
                group by s0.snap_id, s0.END_INTERVAL_TIME, s0.instance_number, s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME, e.tsname
          union all
               select 
                                    s0.snap_id snap_id,
                                    s0.END_INTERVAL_TIME tm,
                      ...
                      sum(e.phyrds - nvl(b.phyrds,0))/ 
                                            ...
                                            + EXTRACT(MINUTE FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) 
                                            + EXTRACT(SECOND FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) / 60, 2))*60) rps,       -- ela
                      decode(sum(e.phyrds - nvl(b.phyrds,0)), 0, to_number(NULL), sum(e.phyblkrd - nvl(b.phyblkrd,0)) / sum(e.phyrds   - nvl(b.phyrds,0)))            bpr,
                      sum(e.writetim  - nvl(b.writetim,0))                 writetim,
                      sum(e.phywrts    - nvl(b.phywrts,0))                 writes,
                      decode(sum(e.phywrts - nvl(b.phywrts,0)), 0, 0, (sum(e.writetim - nvl(b.writetim,0)) / sum(e.phywrts  - nvl(b.phywrts,0)))*10)        atpw,
                      sum(e.phywrts    - nvl(b.phywrts,0)) / 
                      ...
                      decode(sum(e.phywrts - nvl(b.phywrts,0)), 0, 0, sum(e.phyblkwrt - nvl(b.phyblkwrt,0)) / sum(e.phywrts   - nvl(b.phywrts,0)))              bpw,
                      sum(e.wait_count - nvl(b.wait_count,0))              waits,
                      decode(sum(e.wait_count - nvl(b.wait_count, 0)), 0, 0, (sum(e.time - nvl(b.time,0)) / sum(e.wait_count - nvl(b.wait_count,0)))*10) atpwt,
                      sum(e.phyrds  - nvl(b.phyrds,0)) + sum(e.phywrts - nvl(b.phywrts,0))                    ios,
                                    (sum(e.phyrds  - nvl(b.phyrds,0)) + sum (e.phywrts - nvl(b.phywrts,0))) / 
                                            ((round(EXTRACT(DAY FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) * 1440 
                                            + EXTRACT(HOUR FROM s1.END_INTERVAL_TIME - s0.END_INTERVAL_TIME) * 60 
